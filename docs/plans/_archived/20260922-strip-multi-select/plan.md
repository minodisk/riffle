...
# Strip multi-selection and bulk judgments

## Purpose

The strip (`crates/app/ui/src/strip.ts`) has exactly one selected file, the
current one, and every judgment key (`judge` in `crates/app/ui/src/main.ts`)
...
Shift+ArrowUp/Down (extend the range from the keyboard), and every judgment
that works on one file today (stars, reject, pick, un-flag, clear, the seven
color labels and clear-label) applies to every selected file as one undoable
...
- **Same value on every file (user requirement).** A judgment with a
  ...
  selected file (a label key on a mixed selection sets the color on all when
...
- **Unrelated fields are preserved per file (user requirement).** A command
  ...
  own existing value. A star key does not touch the color label (and a label
  key does not touch the stars or the pick / reject flag) of any selected
  file. The per-file `Judgment` written for each target is that file's own
  `before` with only the command's field replaced, never the focused file's
  whole judgment copied over.
- Bulk judgments apply to the selected files that are **visible** (in
  `files`). A file the filter hides is dropped from the selection when the
  filter, sort or a judgment hides it; it is never judged invisibly.
- Auto-advance does not fire after a judgment applied to more than one file;
  the focused file stays where it is (a judgment that hides it under the
...
- [x] Step 1: Pure selection model with tests
  - Done when:
    ...
    - The model also exposes the "targets" of a judgment: the selected paths
...
- [x] Step 2: Strip click modifiers and selection painting
  - Done when:
    ...
    - Painting survives virtualization: a selected cell scrolled out and back
    ...
    - The selection is cleared on folder open (`openDirectory`), pruned on
      every `refilter` (filter, sort, judgment), on `trashRejected` and on
...
- [x] Step 3: Bulk judgments over the selection
  - Done when:
    ...
    - The new value is computed once from the focused file's judgment and
  ...
  - Implementation approach:
    ...
    - Express each command as "which field, which new value" computed from
      ...
      other fields survive; do not copy the focused file's whole `Judgment`
...
- [x] Step 5: Documentation
  - Done when:
    - `README.md` Features (Filmstrip) and Keys mention Cmd/Ctrl+click,
      Shift+click, Shift+ArrowUp/Down and that judgments apply to the whole
      selection
    - `docs/usage.md` Features (Filmstrip, Judgments, Undo, Auto-advance)
