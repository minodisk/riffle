...
- [x] Step 1: Rename the `$label` jq variable and add a stub-`gh` test for the verdict paths
  - Done when:
    ...
    - A new `.claude/skills/merge/scripts/wait-post-merge-runs.test.sh` passes locally and is run by `mise run lint` (so by `mise run ci`), covering at least: all success (exit 0, `STATUS=success`), one failure (exit 1, `STATUS=failed`, `FAILED_COUNT=1`, a `failure` row), canceled with a newer success on the default branch (exit 0, a `superseded` row, `SUPERSEDED_COUNT=1`), and no runs (exit 0, `STATUS=no_runs`)
