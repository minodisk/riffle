...
- The placeholder gray had to move off the loaded image: with a 144x144 box,
  `background` on `.cell img` would paint 24px gray bands above and below a
...
- The visual acceptance criteria (a 3:2 thumbnail looking identical to before,
  ...
  gray) could not be confirmed by eye from this session, which cannot drive the
  GUI. They follow from `object-fit: contain` keeping the intrinsic aspect and
  centering within the box, but the by-eye check is left to the user.
