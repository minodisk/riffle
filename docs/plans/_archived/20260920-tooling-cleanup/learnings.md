...
- The `ai-coauthored` label did exist on GitHub (description "Co-authored with AI", color `#ededed`); it was deleted with `gh label delete ai-coauthored --yes`, which also strips it from past PRs (accepted by the user). `gh label list` no longer shows it.
