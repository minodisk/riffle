...
On macOS eight app items carry an icon. `Open in DxO PhotoLab` and `Check for
...
`Undo`, `Open Folder…`, `Open Log Folder`, `Move Rejected to Trash` and
`Reload Folder` use `IconMenuItem::with_id` with an
