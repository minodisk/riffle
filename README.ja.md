...
- **フィルムストリップ**: 下端にサムネイルが並び、クリックで表示します。`Cmd+クリック` / `Ctrl+クリック` で 1 枚ずつ追加・解除、`Shift+クリック` や `Shift+←` / `Shift+→` で範囲選択でき、判定は選択中のすべてに適用されます。`F6` でフィルムストリップ、`F7` でフォルダーツリー、`F8` でメタペイン、`Tab` で左右両方のペインを隠し、ビューアを広げられます。メタペインの `Maker note` グループには、Sony の AF、ドライブ、手ブレ補正、画作りの設定を表示します。
