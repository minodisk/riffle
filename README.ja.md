...
- **フォーカスマーク**: `f` で、AF 枠を記録するカメラではカメラが使った AF 枠と、そのフォーカス位置の十字を表示します。位置だけを記録するカメラでは十字のみ、AF 位置のないカメラやマニュアルフォーカスの写真では何も表示しません（[カメラが記録する情報](./docs/cameras.md)（英語））。カメラの顔追尾か AF 点の下で検出した顔から AF が顔を捉えたとわかればマークは緑、AF 点の近くに顔が見つかったものの AF 点がどの顔にも乗っていなければオレンジ、Riffle が判断できなければ（AF 点の近くに顔がない）白になります。メタペインにも同じ内容を `Face` 行で表示します。
...
- **連写**: 1 秒以内に撮られたコマはストリップ上で帯と枚数バッジでまとめられます。`ArrowLeft` / `ArrowRight` で連写間を移動、`Alt+ArrowUp` / `Alt+ArrowDown` で連写内のコマを順に移動して端で止まり、`Shift+x` で連写の残りを不採用にします。1 秒未満の撮影時刻を記録しないカメラでは秒単位でまとめます（[カメラが記録する情報](./docs/cameras.md)（英語））。
