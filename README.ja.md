...
- **フォーカスマーク**: `f` でカメラが記録したフォーカス位置に十字を表示します。
- **シャープネス表示**: 各サムネイルの横のバーで、連写のどのコマが最もシャープかがわかります。Sony のボディが顔を追尾していればカメラの瞳 AF 枠で、そうでなければ顔が見つかれば被写体の目で、なければ AF 点の周辺で、それもなければ最もシャープな領域でスコアを取ります。
...
- **撮影情報**: カメラ、レンズ、シャッタースピード、絞り、ISO、焦点距離を表示します。
- **フィルターと並べ替え**: スター、ラベル、縦横、レンズなどで絞り込み、ファイル名、撮影日時、レーティングで並べ替えられます。
- **不採用をゴミ箱へ移動**: `File > Move Rejected to Trash…` で不採用の写真をゴミ箱に移します。削除はしないので、元に戻せば判定も戻ります。
- **取り消し / やり直し**: `Cmd+Z` / `Ctrl+Z` で判定を取り消し、`Shift` を足すとやり直します。
- **自動送り**: 設定でオンにすると、判定するたびに次の写真へ進みます。

詳しい挙動: [docs/usage.md](./docs/usage.md)（英語）
...
|-----|--------|
| `↑` / `↓` | 前 / 次の写真 |
| `Shift+↑` / `Shift+↓` | 選択を前 / 次の写真へ広げる |
| `1`-`5` | スターを付ける |
| `0` | スターを消す |
| `x` | 不採用（スターは保持） |
| `p` | 採用（スターは保持） |
| `u` | 採用 / 不採用を解除 |
| `Ctrl+Alt+1`-`7` | カラーラベル（赤、オレンジ、黄、緑、青、ピンク、紫） |
| `Ctrl+Alt+0` | カラーラベルを消す |
| `c` | スター、採用 / 不採用、カラーラベルをすべて消す |
| `z` | 等倍ピントチェック |
| `g`（長押し） | グレースケール表示 |
| `v` | 選択した写真を比較 / 表示中の写真を連写内で最もシャープなコマと比較 |
| `f` | フォーカスマーク |
| `Cmd+O` / `Ctrl+O` | フォルダーを開く |

キーは `Riffle > Settings...` で変更できます。
...
- **1 秒未満の撮影時刻**: 前のコマから 1 秒以内のコマを連写としてまとめます。記録がないカメラでは秒単位でしかまとめられないため、1 秒をまたぐ連写が分かれたり、直後に撮った別の写真が同じ連写に入ったりすることがあります。
